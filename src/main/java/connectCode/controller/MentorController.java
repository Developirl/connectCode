...
public class MentorController {
	...
	// 멘토 프로필 수정 [학력사항] 페이지
	@PostMapping("mentorEduInfo_Up_first")
	...
	// 멘토 프로필 수정 [학력사항] 페이지
		...
		public String mentorEduInfo_UP(MentorDTO mentor, Model model) {
			...
			for(int i=0; i<school.length; i++) {
				...
				int result = ms.education_update(mentor);
				System.out.println("result  "+ i + "  :  " + result);
			}
	...
	public String mentorExpInfoPage_Up(MentorDTO mentor, Model model) {
		...
		for (int i = 0; i < entering_dates.length; i++) {
		    ...
		    if(temp_departure_date.equals("") || temp_departure_date.equals(" ")) {
	...
	// 멘토 프로필 수정 [기술및분야] 페이지
	@GetMapping("mentorTechInfoPage_Up")
	public String mentorTechInfoPage_Up(MentorDTO mentor) {
		...
		String[] technology = mentor.getCompany().split(",");
		String[] kind = mentor.getEntering_date().split(",");
		String[] license_name = mentor.getDeparture_date().split(",");
		String[] issuing_authority = mentor.getTask().split(",");
		String[] issuing_date = mentor.getTask().split(",");
		
		System.out.println(Arrays.toString(technology));
