			...
			<c:if test="${not empty keyword}">
				<c:if test="${paging.startPage > paging.pageBlk }">
					<li class="page-item"><a class="page-link"
						href="mentoringApplyListPage?page=${paging.startPage - 1}&keyword=${keyword}" style="color: black;">이전</a></li>
				</c:if>
				<c:forEach var="i" begin="${paging.startPage}" end="${paging.endPage}">
					<li <c:if test="${paging.currentPage==i}">class="active"</c:if>><a class="page-link" 
						href="mentoringApplyListPage?page=${i}&keyword=${keyword}" style="color: black;">${i}</a></li>
				</c:forEach>
				<c:if test="${paging.endPage < paging.totalPage}">
					<li class="page-item"><a class="page-link" 
						href="mentoringApplyListPage?page=${paging.endPage + 1}&keyword=${keyword}" style="color: black;" >다음</a></li>
				</c:if>
			</c:if>
			<!-- 전체목록 페이징 처리 -->
			<c:if test="${empty keyword}">
				<c:if test="${paging.startPage > paging.pageBlk }">
				<li class="page-item"><a class="page-link" href="mentoringApplyListPage?page=${paging.startPage - 1}" style="color: black;">이전</a></li>
				</c:if>
				...
				<c:if test="${paging.endPage < paging.totalPage}">
				<li class="page-item"><a class="page-link" 
					href="mentoringApplyListPage?page=${paging.endPage + 1}" style="color: black;">다음</a></li>
				</c:if>
			</c:if>
		</ul>
	
		<%-- <!-- 페이지 버튼 css는 페이징 처리 하는 사람들이랑 맞추기 --> 기존 페이징 ( 검색어 처리를 위한 페이징 전 ..... )
		<ul class="pagination justify-content-center">
